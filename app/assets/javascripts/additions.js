$(function() {
 ...
 $(".submit_player").click(function(e){
   e.preventDefault();
   var name = $("#player").val();
   var row  = $("#" + name.replace(/\s+/g, '').replace(/\./g, '').replace(/\'/,''));
   ...
   $.ajax({
     type: "GET",
     url: url,
     data: {id: id,status:btype}, // serializes the form's elements.
     success: function(data){
       if(btype == "myteam")
         $("#table_myteam").append(data.pstring);
       $('.success_bar')[0].innerHTML = "success";
       $('.success_bar').fadeIn(1000).delay(1000).fadeOut(1000);
       $("#player").val("");
     }
   });
   return false;
